...
const StaffCard = ({
  ...
  return (
    <Row {...props} className='characters-and-staff-card'>
      <Col xs={8} md={4}>
        <img src={staff.image_url} alt='' className='story-card-image'/>
      </Col>
      <Col span={20} className='story-card-info-container p-2'>
        <Link strong onClick={() => window.open(staff.url)}>{staff.name}</Link>
        <Text className='typography-block'>{staff.positions}</Text>
      </Col>
    </Row>
  );
}
